...
const Profile = () => {
  ...
  const fetchPersonalDetails = async () => {
    try {
      const res = await getPersonalDetails(id as string);
        ...
        <div className="flex flex-col gap-1 w-full bg-slate-50 p-5 shadow-form rounded-xl">
          <div className="details flex flex-col gap-4 justify-between px-2 md:px-8">
            ...
            <div className="detailbox w-1/2 flex flex-col gap-2 text-xs">
              <div className="single-details flex flex-col">
                <Typography className="text-gray-600 text-xs  uppercase">
                  Email
                </Typography>
                <Typography className="font-semibold md:text-lg">
          ...
          <div className="stats-top flex justify-between px-2 md:px-8 gap-4 flex-col md:flex-row">
            <div className="statbox flex flex-1 flex-col">
              <Typography className="text-center bg-green-600 items-center rounded-xs text-slate-50 font-semibold">
                Wins
              </Typography>
              <Typography className="text-center">
                {personalDetails?.totalMatchesWon}
              </Typography>
            </div>
            <div className="statbox flex flex-1 flex-col">
              <Typography className="text-center bg-gray-500 items-center rounded-xs text-slate-50 font-semibold">
                Draws
              </Typography>
              <Typography className="text-center">
                {personalDetails?.totalMatchesDrawn}
              </Typography>
            </div>
            <div className="statbox flex flex-1 flex-col">
              <Typography className="text-center bg-red-600 items-center rounded-xs text-slate-50 font-semibold">
                Loses
              </Typography>
              <Typography className="text-center">
                {personalDetails?.totalMatchesLost}
              </Typography>
            </div>
          </div>
          ...
          <div className="stats-bottom px-2 md:px-8 flex flex-col gap-2">
            <div className="flex justify-between">
              ...
              <Typography className="text-sm font-semibold">
                {Math.floor(
                  (personalDetails?.totalMatchesWon /
                    personalDetails?.totalMatchesPlayed) *
                    100
                )}
                %
              </Typography>
